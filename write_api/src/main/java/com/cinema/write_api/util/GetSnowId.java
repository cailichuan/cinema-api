package com.cinema.write_api.util;
...
public class GetSnowId {
   ...
   private static long lastCurrentTimeMillis=1672382200988L;
    ...
    //set设置机器ID和服务ID
    public GetSnowId setSnowIdType(int w){
        Long workId= SnowIdType.getSnowIdTyteByNum(w).getWorkId();
        Long machineId=SnowIdType.getSnowIdTyteByNum(w).getMachineId();
        if (workId<0||workId>maxWorkId){
            throw new IllegalArgumentException("服务id不能小于0或者大于"+maxWorkId);
